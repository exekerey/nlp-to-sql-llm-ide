...
export async function testConnection(signal?: AbortSignal): Promise<InitResponse> {
    ...
    if (!res.ok) {
        try {
            const j = await res.json();
            const msg = j?.detail?.error ?? j?.detail ?? 'Connection test failed';
            throw new Error(typeof msg === 'string' ? msg : 'Connection test failed');
        } catch {
